...
### What is PAWS??
class PAWSResNet50(nn.Module):
    def __init__(self, num_classes):
        '''
            Constructor of the model. Here, we initialize the model's
            architecture (layers).
        '''
        super(PAWSResNet50, self).__init__()
...
#         self.convnet.fc = nn.Identity()                       # discard last layer...
        ...
        self.convnet.fc = nn.Sequential(OrderedDict([
    ...
    def forward(self, x):
        ...
        '''
        # x.size(): [B x 3 x W x H]
        prediction = self.convnet(x)    # features.size(): [B x 512 x W x H]
        #prediction = self.classifier(features)  # prediction.size(): [B x num_classes]
